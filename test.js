...
const QuoteSystem = () => {
  ...
  const handleScratchStart = (e) => {
    ...
    setIsDrawing(true);
    setScratchAnim(false); // stop animation on user interaction
    setIsBouncing(false);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-white to-[#0a174e] flex items-center justify-center p-4 transition-colors duration-1000">
      <div className={`bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl max-w-4xl w-full border border-white/20 transition-all duration-1000 ${
        ...
        {/* Header: Only logo */}
        <div className="relative p-8 border-b border-slate-200/50 flex items-center justify-between bg-blue-600">
          <img src="./logo.png" alt="Atlantic Security Logo" className="w-46 h-20 rounded-xl shadow-lg bg-white" />
          <div className="text-right">
            <div className="text-sm text-slate-100">Valid Until:</div>
            <div className="font-mono text-slate-900 font-semibold">{config.validUntil}</div>
          </div>
        </div>
        {/* Main Content */}
        <div className="p-8">
          ...
          <div className="bg-gradient-to-r from-slate-50 to-blue-50 rounded-2xl p-6 mb-8 border border-slate-200/50">
            <div className="text-center mb-4">
              <div className="text-sm text-slate-500 mb-1">Original Quote Price</div>
              <div className="text-3xl font-bold text-slate-400 line-through">${config.original.toFixed(2)}</div>
            </div>
            
            <div className="flex items-center justify-center mb-4">
              <div className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-6 py-3 rounded-full shadow-lg">
                <div className="text-lg font-bold">Special Offer Below!</div>
                <div className="text-sm opacity-90">Scratch to reveal your exclusive price</div>
              </div>
            </div>
          ...
          <div className="mb-8">
            ...
            <div className="relative w-96 h-60 mx-auto rounded-2xl overflow-hidden shadow-xl animate-card-jump">
                {/* Revealed content */}
                <div className={`absolute inset-0 bg-gradient-to-br from-blue-600 via-indigo-600 to-purple-600 flex flex-col items-center justify-center text-white transition-all duration-700 ${
                  ...
                  <div className="text-5xl font-bold mb-2">{config.discount}% OFF</div>
                  <div className="text-2xl font-bold mb-2">${discountedPrice.toFixed(2)}</div>
          ...
          <div className="text-center mb-8">
            <button
              ...
              disabled={!isRevealed}
              className={`px-8 py-4 text-lg font-semibold rounded-xl transition-all duration-800 ${
                isRevealed 
            ...
            <div className="bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-2xl p-6 mb-8 animate-fade-in">
              <div className="text-center">
                <div className="text-2xl font-bold text-green-700 mb-4">
                  Congratulations! You've unlocked your exclusive offer act now to claim before this unbeatable chance falls through the cracks
                </div>
          ...
          <div className="text-center pt-6 border-t border-slate-200">
            ...
            <div className="flex justify-center space-x-4">
              <a href="tel:+1234567890" className="inline-flex items-center px-4 py-2 text-blue-600 hover:text-white hover:bg-blue-600 rounded-lg transition-all duration-800 font-medium">
      ...
      <style jsx>{`
        ...
        @keyframes card-jump {
          ...
          .animate-card-jump {
            animation: card-jump 0.8s ease-in-out 0s 12;
          }

          .animate-card-bounce {
            animation: card-bounce 12s ease-in-out infinite;
          }
      `}</style>
    </div>
  );
